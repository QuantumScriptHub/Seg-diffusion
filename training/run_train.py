...
def log_validation(vae, text_encoder, tokenizer, unet, args, accelerator, scheduler, epoch):
    ...
    pipeline = SemanticEstimationPipeline.from_pretrained(
    ...
    total_loss = 0.0
    with torch.no_grad():
        with open(args.val_text_list, 'r') as f:
            prompts = json.load(f)
        for img_name in os.listdir(args.val_img):
            ...
            semantic_pred = pipe_out.semantic_np
            gray_img = (semantic_pred * 255).astype(np.uint8)
            gray_img = Image.fromarray(gray_img)
            semantic_pred = torch.from_numpy(semantic_pred).to(accelerator.device)
            semantic_pred = semantic_pred.unsqueeze(0).unsqueeze(0)
            total_loss += F.l1_loss(input_gt, semantic_pred).item()

            semantic_colored = pipe_out.semantic_colored
            ...
            semantic_colored.save(os.path.join(output_path, f"{rgb_name_base}_{epoch}_colored.png"))
            gray_img.save(os.path.join(output_path, f"{rgb_name_base}_{epoch}_gray.png"))
...
def parse_args():
    parser = argparse.ArgumentParser(description="Diffusion-Based Image Generators for Semantic Segmentation")
    parser.add_argument(
    ...
    parser.add_argument(
        "--train_text_list",
        type=str,
        default="train_text.json",
        required=True,
        help="prompt file with a dict, the key is the image name, and the value is the text prompt",
    )
    parser.add_argument(
        "--val_img",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="open-vocabulary semantic segmentation",
    )
    parser.add_argument(
...
def main():
    ...
    # Freeze vae and text_encoder, set unet to trainable.
    ...
    with accelerator.main_process_first():
        train_loader = prepare_dataset(
            ...
            train_text_list=args.train_text_list,
    ...
    # Move vae and text_encoder to gpu and cast to weight_dtype
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_loader):
            with (accelerator.accumulate(unet)):
                semantic_image = batch['original']
                semantic_gt = batch['gt']
                semantic_gt = gt_normalization(semantic_gt)

                # encode image to latents
                h_rgb = vae.encoder(semantic_image.to(weight_dtype))
                ...
                h_mask = vae.encoder(semantic_gt.to(weight_dtype))
                ...
                # Encode text embedding for prompt batch['prompt']
                text_prompt = [x[0] for x in batch['prompt']]
                ...
                noise_pred = unet(unet_input, timesteps, encoder_hidden_states=text_embed).sample
